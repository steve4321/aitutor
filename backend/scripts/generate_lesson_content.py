...
3. explain (讲解): 4-6 分钟, completion_mode: "all_viewed"
   - 讲解概念、公式、定理
   - blocks: 1个 text/callout + 1个 formula + 1个 illustration + 可选 expandable
...
1. warmup (热身): 1-2 分钟, completion_mode: "all_viewed"
   - 回顾相关知识点和公式
   - blocks: 1个 text/callout + 1个 formula(回顾公式)
...
async def generate_for_lesson(lesson: Lesson, dry_run: bool = False) -> dict | None:
    ...
    # Build a dedicated LLM with longer timeout and more tokens for content generation
